...
<figure class="align-center">
  ...
  <figcaption>A rural location in Gloucestershire, with surrounding libraries in nearby towns.</figcaption>
</figure>
...
But you don't need cross-authority bin day lookups, because your local Council does your bin. The same isn't true of libraries. Many people have multiple library services relevant to them.
...
Mobile library stop information can be impossible to find and interpret, even if you're fortunate enough to be on the right website. It needs to first be in a standard data format, and then integrated into library listings.

## Distance is deceptive

Library Map originally allowed users to put in a postcode, and it would show a list of relevant libraries in that area, in alphabetical order. Simple? Maybe - but not what users expect. People expect to see the library closest to their postcode first.
...
Anyway, matching user expectation is important, and it now orders the results by distance. But, all of this is academic. Surely display the information on a map and people can see for themselves where the libraries are? Which leads to the next question...
...
Even beyond the obvious visual barrier, mapping software relies on people being familiar with the map tools - zoom in, zoom out, pan, scroll etc. It's an additional level of expertise that shouldn't be assumed.

So yes, Library Map is not a map. It's mapping libraries in the sense of collecting library locations.

## Surveying libraries is not enough

The data source for libraries in England is the Arts Council [Basic dataset for libraries](https://www.artscouncil.org.uk/supporting-arts-museums-and-libraries/supporting-libraries). This is validated by library services every December, and released as early as possible the following year.
...
As it's now April 2023, it's about 16 months out of date, but the next iteration should be released soon.
...
The problem is, if you're displaying current libraries, a survey doesn't work. You can't say 'well, this is correct as of December 2021'. People are looking for their local library right now.

Library locations should be maintained as data that is always up to date. There are two main ways of doing that:

1. Let library services maintain their own data and everyone publish it in a standard format. I personally think this is the best, but it's by far the most difficult to get everyone to do.
2. Provide an interface for library services to update their data when they need to. This is OK, but it takes data publishing out of the hands of library services, aside from providing a boring task for them to do on someone else's system. Data publishing is important for services to do themselves, and get involved in.

An alternative between the two may be adopting a platform like Wikidata, but libraries would have to accept anyone being able to update the data.
