...
Where are places in England that don't have a public library? Well, there are loads of places without libraries, what does that mean? More clearly - where are the big towns without a library? Or, the towns that could really do with a library that don't have one?
...
Are all current libraries within built-up areas? Not quite - of the 2564 libraries, **6** were outside of these areas. Those were primarily rural libraries. For example, Dent Library Link in Westmorland and Furness is in a location with a population of just 780. There's nothing wrong with this - as [Libraries on the High Street](https://blog.librarydata.uk/libraries-on-the-high-street/) research showed, public libraries appear in all different places, and shouldn't be assumed to be urban. However, such areas are out of scope for this research, as we're primarily focussing on areas of significant population.
...
Those consultations and closures are in the past. But all changes to the network should be assessed on a national basis, as well as a local one. Should there be a population threshold at which we believe places need a library? It's clear that shared tools and data would be useful in assessing library provision. Consultations can be challenged, and are subject to oversight from the DCMS, but even then, at what point does someone step in? Without established guidance on what provision should be, it comes down primarily to ensuring the consultation has been conducted properly.
...
The House of Commons library produced a research report on the classification of towns and cities. It includes specific levels of population that serve to classify places:
...
It would be possible, for example, to have library service standards that specified that a small town should have a library. But leave villages and small communities down to the discretion of the local authority. That would create clear policy on what we expect from library services.

In practice, what would this mean? There are currently **75** built-up areas with a population over 7,500 that currently do not have a statutory library. It would not be a huge undertaking to meet that level of service.
...
The data alongside code and sources for the analysis will be released within the next week.

These insights should be available as permanent dashboards, rather than static reports. The OS Built-Up areas, alongside population and deprivation data will be built into the Libraries Hacked [map tool at LibraryMap](https://www.librarymap.co.uk/#/map). This will allow all users and service professionals to explore different areas and see aspects of missing library provision.
