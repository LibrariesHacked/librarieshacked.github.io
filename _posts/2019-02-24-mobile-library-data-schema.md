...
A [previous post](/mobile-library-data-project) discussed collecting data on mobile libraries in order to improve how this data is used internally, and in providing public information. An encouraging number of library services have been in touch to get involved.

The first step is to analyse existing data, and organise this into a standard. That means deciding the data fields, and additional detail such as whether each field should be a number, or a date, etc. And any constraints on the field such as being mandatory, or having a minimum length. This sort of standard is generally called a **schema**.

It's worth looking around for existing examples. Although there doesn't seem to be anything obviously comparable, there are things to reference:
...
How can we define and document a schema? [Table Schema](https://frictionlessdata.io/specs/table-schema/) is a standard for specifying a table of data. It uses [JSON formatted](https://en.wikipedia.org/wiki/JSON) files for doing this.
...
Additionally, given a **Table Schema** file, CSVLint will check that the CSV matches the schema. Not just that the basic format is correct, but detailed checking of the data definition and constraints. The schema can then become a powerful tool for automatically checking is valid (such as library services mobile stop data).
...
| Timetable | A link to a PDF or web page for the stop, route, or mobile | https://services.wiltshire.gov.uk/MobileLibrary/Library/Stop/209 |

Some notes about the schema:

* There are a few fields dedicated to the location of the stop. Stop name, community, address, postcode, and x/y coordinates. In reality, mobile library stops often do not have addresses or postcodes, as they are not addressable locations. However, many library services publish an address or postcode, normally that of the place the mobile library stop is closest to. So these fields should be optional. The mandatory ones are a stop name and the coordinates.
* The most complex field is probably the frequency. In the example above, FREQ=WEEKLY;INTERVAL=4 means that the mobile library visits every 4 weeks. FREQ=WEEKLY;INTERVAL=2 would be every 2 weeks, and for every week, just a simple FREQ=WEEKLY would be enough. This uses the [Recurrence rule specification](https://icalendar.org/iCalendar-RFC-5545/3-8-5-3-recurrence-rule.html), which is part of creating iCalendar appointments. The benefit of using this specification is that it will make creating downloadable timetables onto a phone calendar easier, as they will understand this format. The majority of mobile stops will be variations on weekly intervals, but some are more complex such as Worcestershire's *3rd Monday in the month*. That's fine though, in the specification that can be specified as FREQ=MONTHLY;BYDAY=1MO (1st Monday each Month). 
...
A follow up post will begin to describe how this data can start being used in practical applications, with more explanation as to why each field is important.
...
It's easy to get carried away. This morning, before getting up, I was imagining how an admin system would work that automatically validated a mobile library's stop data. Not just schema validation as described in this post. But **really** validated it. For example, the system could calculate the route that the mobile library would take between stops and validate that the current departure and arrival timings are realistic. Perhaps just give a warning if it seemed like they needed to be shifted slightly. Even more advanced, it could potentially suggest efficiencies in terms of moving stops around, or switching them to different routes. 

None of this is intended to remove the human expertise behind this - in many cases expertise will have gone in to mobile timetables to decide what times are appropriate in which places (e.g. schools). But technology can be a tool that aids this.